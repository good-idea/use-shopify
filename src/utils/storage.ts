...
export const getCookie = <ExpectedResult>(key: string): ExpectedResult | null => {
...
export const persistData = (key: string, val: any, forceCookie: boolean): void => {
...
export const retrieveData = <ExpectedResult>(key: string, forceCookie: boolean): ExpectedResult => {
	const value = window.localStorage !== null && forceCookie !== true ? window.localStorage.getItem(key) : Cookies.get(key)
	if (value) return JSON.parse(value)
	return null
}
