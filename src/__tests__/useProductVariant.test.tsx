...
describe('useProductVariant', () => {
	...
	it('should throw if the initialVariant does not exist', () => {
		const { result } = renderHook(() => useProductVariant(dummyProduct, { initialVariant: '2' }))
		expect(result.error.message).toBe('There is no variant with the id "2" on the product Dummy Product')
	})
	...
	it('should throw if the product has no variants', () => {
		...
		const { result } = renderHook(() => useProductVariant(dummyProductNoVariants))
