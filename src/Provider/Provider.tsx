...
import { useCheckout, UseCheckoutValues, UseCheckoutQueries, UseCheckoutConfig } from '../useCheckout'
import { useSearch, UseSearchValues, UseSearchQueries, UseSearchConfig } from '../useSearch'
...
export const ShopifyProvider = ({ children, queries, query, config: userConfig }: Props) => {
	const config = {
		...defaultConfig,
		...userConfig,
	}

	const checkout = useCheckout({ queries, query, config: config.checkout ? config.checkout : undefined })
